...
function getDuration(fd) {
    let obj = {};
    let duration = {};
    duration.creationDate = fd.get('creationDate') ? new Date(fd.get('creationDate')).getTime() : 0;
    duration.startDate = fd.get('startDate') ? new Date(fd.get('startDate')).getTime() : 0;
    duration.endDate = fd.get('endDate') ? new Date(fd.get('endDate')).getTime() : 0;
...
function getDutiesObligations(fd) {
    return {
        'Duties/Obligations': {
            'qualityOfData': fd.get('qualityOfData') ? parseInt(fd.get('qualityOfData')) : 0,
